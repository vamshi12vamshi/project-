...
/*5Who is the best customer? The customer who has spent the most money will be
...
FROM track
WHERE milliseconds >(
SELECT AVG(milliseconds) AS avg_songs_per_millisecond
FROM track)
...
SELECT customer.customer_id,customer.first_name,customer.last_name,best_selling.artist_name,
...
JOIN track ON track.track_id=invoice_line.track_id
JOIN album ON album.album_id=track.album_id
JOIN best_selling ON best_selling.artist_id=album.artist_id
GROUP BY 1,2,3,4
ORDER BY 5 DESC;
